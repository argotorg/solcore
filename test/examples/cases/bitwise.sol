...
function fxor(x: word, y: word) returns (word) {
  let acc : word = x ^ y;
  acc ^= x;          // acc = (x ^ y) ^ x == y
  return acc ^ 0;    // identity: a ^ 0 == a
}

function fbitwise(x: word, y: word) returns (word) {
  let acc : word = x & y;
  acc |= x;          // acc = (x & y) | x == x
  acc &= y;          // acc = x & y
  return acc | 0;    // identity: a | 0 == a
...
function fnot(x: word) returns (word) {
...
contract Bitwise {
  ...
  function main() public returns (word) { return fxor(5, 3) ^ fbitwise(6, 3) ^ fnot(4); }
}
