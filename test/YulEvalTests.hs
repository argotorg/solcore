    ...
    [ testCase "an unassigned variable is still inlined" $
      ...
      testCase "a for-loop counter assigned in the post-block is not inlined" $
        let block =
         ...
         in substYulBlock (mkSubst [("i", 0)]) block @?= block,
      testCase "a loop-invariant pre-let shadows the outer literal" $
        let block =
              [ YFor
                  [yLet "i" (yNum 0), yLet "n" (yNum 3)]
                  (yCall "lt" [yIdent "i", yIdent "n"])
                  [yAssign "i" (yCall "add" [yIdent "i", yNum 1])]
                  [yAssign "r" (yIdent "n")]
              ]
         in substYulBlock (mkSubst [("n", 9)]) block @?= block,
