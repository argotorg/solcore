    ...
    = do
        ...
        case mdt of
          Just TDataCon -> do
            ...
            -- nested pairs.
            isT <- isTuple p
            pure $ if isT then mkTuplePat ps'
                    else PCon n ps'
          _ -> do
            addParameter n
            unless (null ps') $ do
...
emptyEnv :: Env
emptyEnv = Env (Map.fromList [ (Name "word", TTyCon)
                             , (Name "bool", TTyCon)
                             , (Name "()", TTyCon)
                             , (Name "->", TTyCon)
                             , (Name "pair", TTyCon)
               ...
               Map.empty
               (Map.fromList [ (Name "true", TDataCon)
                             , (Name "false", TDataCon)
                             , (Name "()", TDataCon)
                             , (Name "pair", TDataCon)
                             , (Name "inl", TDataCon)
...
addTopDecl (S.TClassDef (S.Class _ _ n _ _ sigs)) env
  = let env' = foldr (\ s ac -> let qn = QualName n (pretty (S.sigName s))
                                in Map.insert qn TFunction ac)
                     (scopeEnv env)
                     sigs
    in env{ classEnv = Map.insert n TClass (classEnv env)
          , scopeEnv = env'
          }
addTopDecl (S.TDataDef (S.DataTy n _ cons)) env
  = env { typeEnv = Map.insert n TTyCon (typeEnv env)
        , scopeEnv = foldr (\ d ac -> Map.insert (S.constrName d) TDataCon ac)
                           (scopeEnv env)
                           cons
        }
