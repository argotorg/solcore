...
tcInstance' :: Instance Name -> TcM (Instance Id)
tcInstance' idecl@(Instance d vs ps n ts t funs) =
  do
    checkCompleteInstDef n (map (sigName . funSignature) funs)
    (funs1, _) <- unzip <$> mapM (tcFunDef False vs ps) funs `wrapError` idecl
    instd <- withCurrentSubst (Instance d vs ps n ts t funs1)
    let
      ind@(Instance _ _ ps' _ ts' t' funs2) = everywhere (mkT gen) instd
      vs1 = bv ind
      funs3 =
        sortBy
          ( \f f' ->
    ...
    verifySignatures (Instance d vs1 ps' n ts' t' funs3)
