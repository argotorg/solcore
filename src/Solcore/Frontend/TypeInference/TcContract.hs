...
setupPragmas :: CompUnit Name -> [Pragma] -> TcM ()
setupPragmas (CompUnit imps decls) ps =
  do
    unless validPragmas (invalidPragmaDecl ps)
    mapM_ setupPragma ps
 where
  setupPragma (InferencePragma NoBoundVariableCondition ns) =
    setBoundVariableCondition ns
  setupPragma (InferencePragma NoPattersonCondition ns) =
    setPattersonCondition ns
  setupPragma (InferencePragma NoCoverageCondition ns) =
    setCoverage ns
  setupPragma (BoolTypePragma ns) =
    setupBoolType ns ds

  ds = filter isData decls

  isData (TDataDef _) = True
  isData _ = False
  ...
  unique p xs = single (filter (pragmaType' p) xs)

  pragmaType' p (InferencePragma x _) = p x
  pragmaType' _ _ = False
...
setupBoolType :: NonEmpty Name -> [TopDecl Name] -> TcM ()
setupBoolType ns ds =
  pure ()
