...
import GHC.Stack
import Solcore.Desugarer.StructProjection (fieldSetName, structFieldMap)
...
-- Desugar it to a whole-struct read-modify-write `f = set_x(f, v)`, where
-- set_x reconstructs the struct with field x replaced (generated by
...
transAssignment (FieldAccess (Just (FieldAccess Nothing f)) field) rhs cenv
  | Just fty <- askFieldTy f cenv,
    ...
    field `elem` fieldNames =
      let setter = fieldSetName structName field
          newRhs = Call Nothing setter [FieldAccess Nothing f, rhs]
       in transContractFieldAssignment f newRhs cenv
