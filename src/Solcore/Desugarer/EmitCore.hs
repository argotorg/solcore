...
emitContract c = do
  ...
  case deployer of
    ...
    Just code ->
      let runtimeObject = Core.Object cname runtimeBody []
...
emitConApp con@(Id n ty) as = do
  ...
  case targetType ty of
    ...
    otherType ->
      errors
        [ "emitConApp: not a type constructor: ",
          pretty otherType,
          "\n",
          show otherType
        ]
...
encodeCon n cs (Core.TNamed l t) e = label l (encodeCon n cs t e) -- this will change when we compress tags
  where
    label l (Core.EInl t e) = Core.EInl (Core.TNamed l t) e
...
translatePatArgs e = Map.fromList . go e
  where
    go _ [] = []
    go s [PVar i] = [(idName i, s)]
    go s (PVar i : as) =
      let (s1, s2) = (Core.EFst s, Core.ESnd s)
       in (idName i, s1) : go s2 as
